...
import { addDisplayDetails, addDisplayMovie } from '../utils/movieSlice'
const MovieTrailerAndDetails = () => {
  const dispatch = useDispatch()
  const movie = useSelector((store) => store.movies.displayMovie)
  const trailer = useSelector((store) => store.movies.displayDetails)
  useTrailer(movie.id, false)

  const closeActive = () => {
    dispatch(addDisplayMovie(null))
    dispatch(addDisplayDetails(null))
  }
        ...
        <div className="  bg-black w-11/12 px-5  py-10 border-2 border-gray-600 rounded">
          ...
          <div className="flex flex-wrap  md:flex-nowrap">
            ...
            <div className="ml-5 p-4 md:w-5/12">
              ...
              <p className="mt-4 text-gray-100 text-sm">{movie?.overview}</p>
              <div className="text-gray-50 mt-2 font-semibold">
                {movie?.vote_average} Avg. Ratings
              </div>
            </div>
          </div>
        </div>
...
export default MovieTrailerAndDetails
